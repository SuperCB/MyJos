...
page_alloc(int alloc_flags) {
    ...
    struct PageInfo *temp = page_free_list->pp_link;

    page_free_list = temp;

    char *page = page2kva(res);
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store) {
    // Fill this function in
    pte_t *res_pte = pgdir_walk(pgdir, va, 0);
    if (res_pte == NULL)
        return NULL;
    if (pte_store != NULL) {
        (*pte_store) = res_pte;
    }
    return pa2page(PTE_ADDR(*res_pte));
}
...
page_remove(pde_t *pgdir, void *va) {
    // Fill this function in
    pte_t *res = pgdir_walk(pgdir, va, 0);
    if (res == NULL)
        return;
...
mmio_map_region(physaddr_t pa, size_t size) {
    ...
    uint32_t last_base = base;
    base = ROUNDUP(base + size, PGSIZE);

    if (base > MMIOLIM) {
        panic("the reservation would overflow MMIOLIM");
    }
