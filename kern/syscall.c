...
sys_exofork(void) {
    ...
    struct Env *env;
    int f;
    if ((f = env_alloc(&env, curenv->env_id)) < 0) {
        return f;
    }
...
sys_env_set_status(envid_t envid, int status) {
    ...
    if (!(env->env_status == ENV_RUNNING || env->env_status == ENV_NOT_RUNNABLE)) {
        return -E_INVAL;
    }
...
sys_page_alloc(envid_t envid, void *va, int perm) {
    ...
    int flag = page_insert(env->env_pgdir, pp, va, perm);
...
sys_page_map(envid_t srcenvid, void *srcva,
    ...
    pte_t *pte_item;
    struct PageInfo *pp;
    if (!(pp = page_lookup(srcenv->env_pgdir, srcva, &pte_item))) {
        return -E_INVAL;
    }
...
//        cprintf("envid %d\n", envid);
    ...
    if ((uint32_t) srcva < UTOP) {
        ...
        if ((r = page_insert(env->env_pgdir, pp, env->env_ipc_dstva, (int32_t) perm)) < 0) {
            return r;
        }
        flag = true;
    }
